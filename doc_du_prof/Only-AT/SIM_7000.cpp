...
SIM_7000::SIM_7000(Stream *s):DFRobot_SIMcore(s)
{
	_s = s;
}

bool SIM_7000::setBaudRate(long rate)
{
  int  count = 0;
  while(count <3){
    if( rate  ==  1200){
...
bool SIM_7000::checkSIMStatus(void)
{
  int count = 0;
  while(count < 3){
    if(checkSendCmd("AT\r\n","OK")){
...
bool SIM_7000::setNetMode(eNet net)
{
  if(net == eNB){
    _mode_t=0;
...
bool SIM_7000::attacthService(void)
{
  char gprsBuffer[32];
...
int SIM_7000::checkSignalQuality(void)
{
...
bool SIM_7000::openNetwork(eProtocol ptl,const char *host, int port)
{
  char num[4];
  char resp[96];
  if(ptl == eTCP){
...
bool SIM_7000::send(char *buf, size_t len)
{
  char num[4];
  itoa(len, num, 10);
  sendCmd("AT+CIPSEND=");
...
bool SIM_7000::mqttConnect(const char* iot_client, const char* iot_username, const char* iot_key)
{
...
bool SIM_7000::mqttPublish(const char* iot_topic, String iot_data)
{
  if(checkSendCmd("AT+CIPSEND\r\n",">")){
    DBG("aa");
...
bool SIM_7000::mqttSubscribe(const char* iot_topic)
{
  if(checkSendCmd("AT+CIPSEND\r\n",">")){
    char MQTTbuff[10]={0};
    MQTTbuff[0] = 0x82;
...
bool SIM_7000::mqttUnsubscribe(const char* iot_topic)
{
  if(checkSendCmd("AT+CIPSEND\r\n",">")){
    char MQTTbuff[10]={0};
    MQTTbuff[0] = 0xa2;
...
bool SIM_7000::mqttRecv(char* iot_topic, char* buf, int maxlen)
{
...
bool SIM_7000::mqttDisconnect(void)
{
  if(checkSendCmd("AT+CIPSEND\r\n",">")){
...
bool SIM_7000::httpInit(eNet net)
{
  if(net == eNB){
...
bool SIM_7000::httpConnect(const char *host)
{
  httpDisconnect();
  delay(1000);
...
bool SIM_7000::httpPost(const char *host , String data)
{
...
void SIM_7000::httpGet(const char *host)
{
  char resp[40];
  if(!httpConnect(host)){
    return;
...
void SIM_7000::httpDisconnect(void)
{
  sendCmd("AT+HTTPTERM\r\n");
}

int SIM_7000::recv(char* buf, int maxlen)
{
...
bool SIM_7000::getPosition(void)
{
  char posBuffer[150];
  char *position;
...
const char* SIM_7000::getLatitude(void)
{
...
const char* SIM_7000::getLongitude(void)
{
  if(getCommandCounter() >= 5){
    setCommandCounter(6);
    return _longitude;
...
bool SIM_7000::closeNetwork(void)
{
...
int SIM_7000::batteryPower(void)
{
  char batteryBuff[26];
  char *pBattery;
  int i=0,j=0,k=0;
